...
搜索: `js:
    try {
        let filterVodIdKeywords = "游戏,原创,纪录片,片花,教育,健康,军事,脱口秀,母婴,娱乐,生活,财经,体育,科技,搞笑,音乐,旅游,时尚,资讯,微剧,漫剧";
        let vodIdKeywords = filterVodIdKeywords.split(',').map(k => k.trim()).filter(k => k !== '');
        
        let filterVodNameKeywords = "采访,特辑,混剪,动态漫画,《,纪录片,片段";
        let vodNameKeywords = filterVodNameKeywords.split(',').map(k => k.trim()).filter(k => k !== '');
        ...
        try {
            ...
            let match = input.match(/key=([^&]+)/);
            if (match) {
                searchKey = decodeURIComponent(match[1]);
            }
        }
        
        let html = request(input);
        let json = JSON.parse(html);
        let docs = json.data.docinfos;
        
        if (vodIdKeywords.length > 0 || vodNameKeywords.length > 0) {
            docs = docs.filter(item => {
                let info = item.albumDocInfo;
                
                let vod_id = "";
                try {
                    vod_id = info.channel + "$" + info.albumId;
                } catch(e) {
                    vod_id = "";
                }
                
                let vod_name = info.albumTitle || "";
                
                for (let keyword of vodIdKeywords) {
                    if (vod_id.includes(keyword)) {
                        return false;
                    }
                }
                
                for (let keyword of vodNameKeywords) {
                    if (vod_name.includes(keyword)) {
                        return false;
                    }
                }
        ...
        if (searchKey && searchKey.trim() !== '') {
            ...
            docs = docs.filter(item => {
                let info = item.albumDocInfo;
                let vod_name = info.albumTitle || "";
                return vod_name.toLowerCase().includes(keyword.toLowerCase());
            });
        }
        ...
        docs.forEach(item => {
            ...
            d.push({
                ...
                desc: info.tvFocus || "",
                pic_url: info.albumVImage || "",
                url: info.channel + "$" + info.albumId
            });
        });
